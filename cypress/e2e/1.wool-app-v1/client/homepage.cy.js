	...
	context(`Desktop 1080p`, () => {
		...
		it(`(+)Verify that user can access the homepage`, () => {
			...
			cy.get(".css-2us6ol")
				...
				.should(($lis) => {
					...
					expect($lis.eq(2), "Thrid item").to.contain("Enneagram");
					expect($lis.eq(3), "Fourth item").to.contain("Counseling");
					expect($lis.eq(4), "Fifth item").to.contain("Transaction");
					expect($lis.eq(5), "Sixth item").to.contain("DMHP");
					expect($lis.eq(6), "Seventh item").to.contain("CV Builder");
				});
			cy.log("sidebar Active");
			cy.get(".css-2us6ol > :nth-child(1)").should("have.class", "css-1f8k0bu");
			cy.log("sidebar not Active");
			cy.get(".css-2us6ol > :nth-child(2)").should("have.class", "css-1kyqmic");
		...
		it(`(+)Verify that user can view the enneagram test result`, () => {
			...
			cy.get("@enneagramHomepage").then((text2) => {
				cy.get(".css-x9o02n > .chakra-heading").should("have.text", text2);
			});
		});
		...
		it.only(`(+)Verify that user can view user's coach`, () => {
			...
			cy.get(".css-bco1gb > .css-di2piv").should("contain", "Life Coach");
